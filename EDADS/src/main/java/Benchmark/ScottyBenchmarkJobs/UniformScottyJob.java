...
public class UniformScottyJob<S extends MergeableSynopsis> {
    ...
    public UniformScottyJob(String configuration, List<Window> assigner, StreamExecutionEnvironment env, final long runtime,
        ...
        if (stratified) {
            messageStream = env
                    .addSource(new UniformDistributionSource(runtime, throughput, gaps, env.getParallelism()));
        } else {
            messageStream = env
...
//			synopsesStream = BuildStratifiedSynopsis.scottyWindows(timestamped, windows, 0, 0, synopsisClass, parameters);
            synopsesStream = BuildStratifiedSynopsis.scottyWindows(timestamped, windows, new RichStratifier(env.getParallelism(), true), synopsisClass, parameters);
        } else {
